...
function p4_build {
    ~/tools/p4_build.sh ~/p4dra/TNA/p4src/tofino/p4dra.p4 $@
}
...
function topo_start {
    ...
    sleep 2
    screen -dm -S "tofino-model" ./run_tofino_model.sh -p p4dra --arch tofino
    sleep 5
    screen -dm -S "bf_switchd" ./run_switchd.sh -p p4dra --arch tofino
    sleep 5
    screen -dm -S "bfshell" ./run_bfshell.sh -b ~/p4dra/TNA/bfrt_python/setup.py -i
    sleep 5
...
#Start the topology with optee running
function topo_start_optee {
	#p4_build
	sudo ~/tools/veth_setup.sh
    sleep 3
    sudo sysctl -w net.ipv6.conf.all.disable_ipv6=1
    ...
    sudo ip netns exec h1 ip link set addr 08:00:00:00:01:01 dev veth1
    sudo ip netns exec h1 ip link set up dev veth1
    sudo ip netns exec h1 ip neigh add 10.0.2.16 lladdr 52:54:00:00:00:01 dev veth1
    sudo ip netns exec h1 ip route add default via 10.0.1.2
    sudo ip netns exec h1 sysctl -w net.ipv6.conf.all.disable_ipv6=1
}
