...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr, "Uso: %s <arquivo> <interface> [quantidade configuracoes]\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    for (int id = 0; id < quantidade_configuracoes; id++) {
        ...
        // Convertendo ID_dispositivo e Chave em Bytes e já colocando na estrutura do cabeçalho do P4DRA
        // Invertendo Endianess
        swapEndian(dados[id].id_dispositivo, ID_DISPOSITIVO_LEN);
        swapEndian(dados[id].chave, CHAVE_LEN);

        // Conversão Hexstring->Bytes
        hexstring_to_byte_array(custom_hdr.id_dispositivo, dados[id].id_dispositivo, ID_DISPOSITIVO_LEN);
        hexstring_to_byte_array(custom_hdr.chave, dados[id].chave, CHAVE_LEN);
