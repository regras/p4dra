...
#define ID_MAX 200
uint32_t quantidade_configuracoes = 1000;
...
struct custom_header {
    uint8_t oper;
    uint8_t id_rodada[NONCE_LEN_BYTES];
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr, "Uso: %s <arquivo_provas> <interface> [quantidade configuracoes]\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    for (int id = 0; id < quantidade_configuracoes; id++) {
        ...
        if (hexstring_to_byte_array(custom_hdr.id_dispositivo, dados[id].id_dispositivo, ID_DISPOSITIVO_LEN) != 0 ||
            hexstring_to_byte_array(custom_hdr.chave, dados[id].chave, CHAVE_LEN) != 0) {
        ...
        //Zerando ID_Rodada (nao utilizado no setup)
        for (int i = 0; i < NONCE_LEN_BYTES; i++) {
            custom_hdr.id_rodada[i]=0;
        }
